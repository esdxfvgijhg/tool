...
#        manager.ensure_update()
    ...
    def do_activate(self):
        ...
        # Insert menu items
#        self._insert_menu()
        action = Gio.SimpleAction(name="align")
        action.connect('activate', self.do_align_activate)
...
#        self._remove_menu()
        self.window.remove_action("align")
...
#        self._action_group.set_sensitive(doc is not None)
        self.window.lookup_action("align").set_enabled(doc is not None)
    ...
    def align(self, doc, bounds, separator):
        ...
        lines = list(range(bounds[0].get_line(), bounds[1].get_line() + 1))
...
class AlignAppActivatable(GObject.Object, Gedit.AppActivatable):
    ...
    def do_activate(self):
        self.app.add_accelerator("<Ctrl><Alt>A", "win.align", None)

        self.menu_ext = self.extend_menu("tools-section")
        item = Gio.MenuItem.new(_("Align"), "win.align")
        self.menu_ext.append_menu_item(item)

    def do_deactivate(self):
        self.app.remove_accelerator("win.align", None)
        self.menu_ext = None
