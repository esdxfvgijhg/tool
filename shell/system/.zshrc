...
# /etc/motd
if [ -e /etc/motd ]; then
	if ! cmp -s $HOME/.config/autostart/hushlogin /etc/motd; then
		tee $HOME/.config/autostart/hushlogin < /etc/motd
	fi
fi
...
for file in ~/Aghiles/tool/shell/system/{export,functions,alias,zsh-functions}; do
	[ -r "$file" ] && source "$file"
done
unset file
